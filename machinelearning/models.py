...
class PerceptronModel(object):
    ...
    def train(self, dataset):
        ...
        while temp:
        	temp = False
        	val = 1
        	tempor = dataset.iterate_once(val)
        	for i, j in tempor:
        		j = nn.as_scalar(j)
        		if self.get_prediction(i) != j:
        			temp = True
        			self.w.update(i, j)
...
class RegressionModel(object):
    ...
    def train(self, dataset):
        ...
        """
        "*** YOUR CODE HERE ***"
        temp = dataset.iterate_forever(self.b_size)
        for x,y in temp:
        	...
        	if nn.as_scalar(self.get_loss(x,y)) < 0.01:
        		...
        		m = m * -1
        		temp1 = nn.gradients(self.get_loss(x,y), [self.w1, self.w2, self.b1, self.b2])
        		self.w1.update(temp1[0], m)
        		self.w2.update(temp1[1], m)
        		self.b1.update(temp1[2], m)
        		self.b2.update(temp1[3], m)
...
class DigitClassificationModel(object):
    ...
    def train(self, dataset):
        ...
        for x,y in temp:
        	...
        	self.b1.update(temp1[2], self.learning_rate)
        	self.b2.update(temp1[3], self.learning_rate)
